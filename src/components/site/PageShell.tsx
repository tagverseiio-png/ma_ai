...
export const PageShell = ({ index, eyebrow, title, accent, intro, children }: PageShellProps) => {
  return (
    <>
      ...
      <div className="min-h-screen bg-[#F4F4F0] text-[#111111] selection:bg-[#CCFF00]/30">
        <SiteNav />
        <main>
            ...
            <div className="max-w-[1400px] mx-auto px-5 md:px-12 relative z-10">
              <motion.div initial="hidden" animate="visible" variants={fadeInUp}>
                <div className="flex items-center gap-4 mb-8">
                  <span className="text-[#111111] font-medium text-sm tracking-widest">{index}</span>
                  <div className="h-[1px] w-16 bg-[#CCFF00]/50" />
                  <span className="text-gray-600 text-[11px] tracking-[0.2em] uppercase font-semibold">{eyebrow}</span>
                </div>
                <h1 className="text-[34px] sm:text-[52px] md:text-[84px] font-bold leading-[1.03] tracking-[-0.02em] max-w-[900px]">
                  {title}{' '}
                  <span className=" text-[#111111]">{accent}</span>
                </h1>
                <p className="text-gray-600 text-[18px] md:text-[20px] font-light leading-[1.6] max-w-[620px] mt-8">{intro}</p>
              </motion.div>
            </div>
          </section>
          {children}
...
export const Section = ({ children, className = '' }: { children: ReactNode; className?: string }) => (
  <section className={`py-20 border-t border-black/10 ${className}`}>
