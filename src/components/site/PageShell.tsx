...
export const PageShell = ({ index, eyebrow, title, accent, intro, children }: PageShellProps) => {
  return (
    <>
      ...
      <div className="min-h-screen bg-[#05050A] text-white selection:bg-[#8B5CF6]/30">
        <SiteNav />
        <main>
          <section className="relative pt-48 pb-24 overflow-hidden">
            <div className="absolute top-[-30%] right-[-10%] w-[700px] h-[700px] bg-[#4F46E5]/20 blur-[160px] rounded-full pointer-events-none" />
            ...
            <div className="max-w-[1400px] mx-auto px-5 md:px-12 relative z-10">
              <motion.div initial="hidden" animate="visible" variants={fadeInUp}>
                <div className="flex items-center gap-4 mb-8">
                  <span className="text-[#8B5CF6] font-medium text-sm tracking-widest">{index}</span>
                  <div className="h-[1px] w-16 bg-[#8B5CF6]/50" />
                  <span className="text-gray-400 text-[11px] tracking-[0.2em] uppercase font-semibold">{eyebrow}</span>
                </div>
                <h1 className="text-[34px] sm:text-[52px] md:text-[84px] font-bold leading-[1.03] tracking-[-0.02em] max-w-[900px]">
                  {title}{' '}
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#8B5CF6] to-[#4F46E5]">{accent}</span>
                </h1>
                <p className="text-gray-400 text-[18px] md:text-[20px] font-light leading-[1.6] max-w-[620px] mt-8">{intro}</p>
              </motion.div>
            </div>
          </section>
          {children}
