          ...
          {roles.map((r, i) => (
            ...
            >
              <div>
                ...
                <div className="flex flex-wrap items-center gap-4 text-[13px] text-[var(--site-muted)]">
                  <span className="text-[var(--site-fg)]">{r.team}</span>
                  <span className="flex items-center gap-1.5"><MapPin size={14} /> {r.location}</span>
                  <span>{r.type}</span>
                </div>
              </div>
              <Link to="/contact" className="inline-flex items-center gap-3 px-6 py-3 rounded-full border border-[var(--site-border)] font-semibold text-[14px] hover:bg-[var(--site-surface)] hover:text-black transition-all w-fit">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {perks.map((p) => (
            <div key={p.title} className="glass-card rounded-[22px] p-8">
              ...
              <p className="text-[var(--site-muted)] font-light leading-[1.6]">{p.desc}</p>
            </div>
          ))}
        </div>
      </Section>
